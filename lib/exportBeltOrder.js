...
import { effectiveGradingBelt, displayNextRank } from "./gradingBelt";
...
export async function exportBeltOrderPdf(students, options) {
  ...
  for (const [size, list] of grouped) {
    ...
    for (const s of list) {
      ...
      const belt = effectiveGradingBelt(s, category);
      const beltLabel =
        belt === "unknown" ? "Needs review" : `${beltDisplayName(belt)} belt`;
      doc.text(`• ${s.fullName} — grading to ${beltLabel}`, margin + 2, y);
      y += 5.5;
    }
    y += 4;
  }
...
export function exportBeltOrderCsv(students, options) {
  ...
  const rows = sorted.map((s) => {
    const grading = effectiveGradingBelt(s, category);
    return [
      s.fullName,
      s.beltSize || "",
      s.currentRank || "",
      displayNextRank(s, category),
      grading === "unknown" ? "Needs review" : beltDisplayName(grading),
      s.email || "",
      s.phone || "",
    ];
  });
