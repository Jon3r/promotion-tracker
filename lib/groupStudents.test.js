...
describe("groupByBelt", () => {
  ...
  it("groups kids by next belt for grading PDF", () => {
    const roster = [
      {
        ...student("grey", 2, "grey", 3),
        fullName: "Stripe on Grey",
        currentParsed: normaliseRank("Grey Belt 2 stripe"),
        nextParsed: normaliseRank("Grey Belt 3 stripe"),
      },
      {
        ...student("grey", 2, "greywhite", null),
        fullName: "To Grey White",
        currentParsed: normaliseRank("Grey Belt 2 stripe"),
        nextParsed: normaliseRank("Grey/White Belt"),
      },
    ];

    const byCurrent = groupByBelt(roster, "kids", "name");
    expect(byCurrent.get("grey")).toHaveLength(2);

    const byNext = groupByBelt(roster, "kids", "name", { groupByNext: true });
    expect(byNext.get("grey")).toHaveLength(1);
    expect(byNext.get("greywhite")).toHaveLength(1);
    expect(byNext.get("greywhite")[0].fullName).toBe("To Grey White");
  });

  it("groups William-style orange black → green as greenwhite", () => {
    const roster = [
      {
        ...student("orangeblack", null, "green", null),
        fullName: "William Atton",
        currentParsed: normaliseRank("Orange Black Belt"),
        nextParsed: normaliseRank("Green Belt"),
        nextRank: "Green Belt",
      },
    ];

    const byNext = groupByBelt(roster, "kids", "name", { groupByNext: true });
    expect(byNext.get("greenwhite")).toHaveLength(1);
    expect(byNext.get("greenwhite")[0].fullName).toBe("William Atton");
  });
});
