...
import { supportsGradingBeltView } from "./gradingBelt";
...
function renderCategoryPdf(doc, students, options, logoDataUrl) {
  ...
  const pageW = doc.internal.pageSize.getWidth();
  const pageH = doc.internal.pageSize.getHeight();
  const footerReserve = ceremonyDate ? 22 : 0;
  let y = drawPdfHeader(doc, category, logoDataUrl, { gradingDate });

  const groupOptions =
    options.groupByGrading && supportsGradingBeltView(category)
      ? { groupByGrading: true }
      : {};
  const grouped = groupByBelt(students, category, "name", groupOptions);
...
/**
 * @param {import('./parseExcel').Student[]} students
 * @param {{ category: string, filename?: string, gradingDate?: string, ceremonyDate?: string, groupByGrading?: boolean }} options
 */
export async function exportNamesPdf(students, options) {
  const { category, filename, gradingDate, ceremonyDate, groupByGrading } =
    options;
  ...
  renderCategoryPdf(
    doc,
    students,
    { category, gradingDate, ceremonyDate, groupByGrading },
    logoDataUrl
  );
  doc.save(
    filename ||
 ...
 *   groupAdultsByGrading?: boolean,
 * }} options
 */
export async function exportBothCategoriesPdf(options) {
  const { adults, kids, eventDates, groupAdultsByGrading } = options;
  const logoDataUrl = await loadLogoDataUrl();
  const doc = new jsPDF({ unit: "mm", format: "a4" });
  ...
  if (adults.length) {
    renderCategoryPdf(
      ...
      adults,
      {
        ...
        groupByGrading: groupAdultsByGrading,
      },
      logoDataUrl
    );
  }
