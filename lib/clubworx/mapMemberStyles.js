...
/**
 * ClubWorx may return booleans as native booleans, numbers, or strings.
 * @param {unknown} value
 * @returns {boolean}
 */
 ...
 * @returns {boolean}
 */
function hasReadyToPromoteAlert(record, nextRank) {
  const flags = [
    record.ready_to_promote,
    record.ready_for_promotion,
    record.is_ready_to_promote,
    record.promote_ready,
  ];
  if (flags.some(parseBooleanish)) return true;
  return /ready to promote/i.test(nextRank);
}
...
export function memberStyleToStudent(record, memberByContactKey, excludedStatuses) {
  ...
  return {
    ...
    readyToPromote: hasReadyToPromoteAlert(record, nextRank),
    promotionDate: null,
