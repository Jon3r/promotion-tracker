 ...
 * @param {string} contactKey
 */
export async function deleteGradingOverride(category, contactKey) {
  const sql = await getSql();
  if (!sql) throw new Error("Database not configured");

  await sql`
    DELETE FROM roster_overrides
    WHERE category = ${category} AND contact_key = ${contactKey}
  `;
}
 ...
 * @returns {Promise<{ adults: Record<string, string>, kids: Record<string, string> }>}
 */
export async function getGiSizeOverrides() {
  const sql = await getSql();
  if (!sql) return { adults: {}, kids: {} };

  const rows = await sql`
    SELECT contact_key, category, gi_size
    FROM roster_overrides
    WHERE gi_size IS NOT NULL AND gi_size <> ''
  `;

  const adults = {};
  const kids = {};
  for (const row of rows.rows) {
    if (row.category === "kids") kids[row.contact_key] = row.gi_size;
    else adults[row.contact_key] = row.gi_size;
  }
  return { adults, kids };
}
 ...
 * @param {string} giSize
 */
export async function upsertGiSizeOverride(category, contactKey, giSize) {
  const sql = await getSql();
  if (!sql) throw new Error("Database not configured");

  await sql`
    INSERT INTO roster_overrides (contact_key, category, gi_size, updated_at)
    VALUES (${contactKey}, ${category}, ${giSize}, NOW())
    ON CONFLICT (contact_key, category) DO UPDATE SET
      gi_size = EXCLUDED.gi_size,
      updated_at = NOW()
  `;
}
