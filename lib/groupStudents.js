 ...
 */
function beltKeyForStudent(student, options = {}) {
  if (options.groupByGrading) return effectiveGradingBelt(student);
...
export function groupByBelt(students, category, sortBy = "date", options = {}) {
  ...
  for (const student of students) {
    const belt = beltKeyForStudent(student, options);
 ...
 */
export function filterByGradingBeltKey(students, beltFilter) {
  if (!beltFilter || beltFilter === "all") return students;
  return students.filter((s) => effectiveGradingBelt(s) === beltFilter);
}
...
export function gradingBeltFilterOptions(students, category) {
  const belts = new Set();
  for (const s of students) {
    belts.add(effectiveGradingBelt(s));
  }
  return [...belts].sort(
    (a, b) => beltSortIndex(a, category) - beltSortIndex(b, category)
  );
...
export function applyFilters(
  students,
  { search = "", beltFilter = "all", stripeFilters = [], viewMode = "current" } = {}
) {
  let list = filterBySearch(students, search);
  if (viewMode === "grading") {
    list = filterByGradingBeltKey(list, beltFilter);
  } else {
