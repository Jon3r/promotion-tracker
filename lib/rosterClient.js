...
export async function fetchClubWorxClasses(day) {
  const query = day ? `?day=${encodeURIComponent(day)}` : "";
  const res = await fetch(`/api/clubworx/classes${query}`);
  const data = await res.json();
  if (!res.ok) {
    return { ok: false, error: data.error, classes: [] };
  }
  return { ok: true, classes: data.classes || [] };
}

export async function fetchClubWorxClassAttendees(classId) {
  const res = await fetch(`/api/clubworx/classes/${encodeURIComponent(classId)}/attendees`);
  const data = await res.json();
  if (!res.ok) {
    return { ok: false, error: data.error, attendeeContactKeys: [] };
  }
  return {
    ok: true,
    attendeeContactKeys: data.attendeeContactKeys || [],
